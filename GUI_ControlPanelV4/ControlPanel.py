...
root = Tk()
root.wm_title("SmartifyBot ControlPanel")
  ...
  if (cmdType == "If Inp"):
    ...
    if(querry == valNum):
      if(action == "Call"):
        ...
        progName = str(command[progIndex+5:]) + ".prog" 
        callProg(progName)
        time.sleep(.4) 
        index = 0  
        tab1.progView.selection_clear(0, END)
  ...
  if (cmdType == "If COM"):
    ...
    if (curCOMVal == valNum):
      if(action == "Call"):
        ...
        tab1.lastProg = ProgEntryField.get()
        progIndex = command.find("Prog")
        progName = str(command[actionIndex+12:]) + ".prog" 
        callProg(progName)
        time.sleep(.4) 
        index = 0  
        tab1.progView.selection_clear(0, END)
...
def loadProg():
  ...
  #folder = os.path.dirname(os.path.realpath(__file__))
  filetypes = (('robot program', '*.prog'),("all files", "*.*"))
...
def insertCallProg():  
  ...
  if  str(changeProg[-4:]) != ".prog":
    changeProg = changeProg + ".prog"             
...
def savePosData():
  ...
  pickle.dump(value,open("GUI_ControlPanelV4//SmartifyBot.cal","wb"))
...
loadDefaultBut = Button(tab3,  text="Load Default",  width=26, command = LoadDefault)
loadDefaultBut.place(x=1150, y=510)
...
VisBackdropImg = ImageTk.PhotoImage(Image.open("GUI_ControlPanelV4//VisBackdrop.png"))
...
playGPhoto=PhotoImage(file="GUI_ControlPanelV4//play-icon.png")
...
try:
  Cal = pickle.load(open("GUI_ControlPanelV4//SmartifyBot.cal","rb"))
except:
  Cal = "0"
  pickle.dump(Cal,open("GUI_ControlPanelV4//SmartifyBot.cal","wb"))
for item in Cal:
