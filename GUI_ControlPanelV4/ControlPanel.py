...
def darkTheme():
  ...
  #                foreground="white",
  #                fieldbackground="#000")
  # style.configure('TNotebook', font='Arial Bold', foreground='salmon')
    ...
    while tab1.runTrue == 1:
      if (tab1.runTrue == 0):
        if (estopActive == TRUE):
          ...
          almStatusLab2.config(text="Estop Button was Pressed")
        elif (posOutreach == TRUE):
          almStatusLab.config(text="Position Out of Reach")
          almStatusLab2.config(text="Position Out of Reach")  
        else:
          almStatusLab.config(text="PROGRAM STOPPED")
          almStatusLab2.config(text="PROGRAM STOPPED")  
      else:
        almStatusLab.config(text="PROGRAM RUNNING")
        almStatusLab2.config(text="PROGRAM RUNNING") 
      rowinproc = 1
      executeRow()
      while rowinproc == 1:
        time.sleep(.1)	  
 ...
 ##Offs J Command##  
  ...
  if (cmdType == "Move A"):
    ...
    if (subCmd == "Move A End"):
      almStatusLab.config(text="Move A must start with a Mid followed by End")
      almStatusLab2.config(text="Move A must start with a Mid followed by End")
    else:
      xIndex = command.find(" X ")
      yIndex = command.find(" Y ")
      zIndex = command.find(" Z ")
  ...
  if (cmdType == "Move C"):
    ...
    if (subCmd == "Move C Sta" or subCmd == "Move C Pla"):
      almStatusLab.config(text="Move C must start with a Center followed by Start & Plane")
      almStatusLab2.config(text="Move C must start with a Center followed by Start & Plane")
    else:
      xIndex = command.find(" X ")
      yIndex = command.find(" Y ")
      zIndex = command.find(" Z ")
...
def xbox():
  def threadxbox():
    ...
    if xboxUse == 0:
      ...
      almStatusLab2.config(text='JOGGING JOINTS 1 & 2')
      xbcStatusLab.config(text='Xbox ON', )
      ChgDis(2)
    else:
      xboxUse = 0
      almStatusLab.config(text='XBOX CONTROLLER OFF')
      almStatusLab2.config(text='XBOX CONTROLLER OFF')
    ...
    while xboxUse == 1:
        ...
        for event in events:
          ...
          ##JOINT MODE
            ...
            if mainMode != 1:
              ...
              jogMode +=1        
            if jogMode == 2:
              almStatusLab.config(text='JOGGING JOINTS 3 & 4')
              almStatusLab2.config(text='JOGGING JOINTS 3 & 4')
            elif jogMode == 3:
              almStatusLab.config(text='JOGGING JOINTS 5 & 6')
              almStatusLab2.config(text='JOGGING JOINTS 5 & 6')
            elif jogMode == 4:
              jogMode = 1
              almStatusLab.config(text='JOGGING JOINTS 1 & 2')
              almStatusLab2.config(text='JOGGING JOINTS 1 & 2')
         ...
         ##CARTESIAN DIR MODE
            ...
            if mainMode != 2:
              mainMode = 2
              jogMode = 1
              almStatusLab.config(text='JOGGING X & Y AXIS')
              almStatusLab2.config(text='JOGGING X & Y AXIS')
            else:                
              jogMode +=1        
            if jogMode == 2:
              almStatusLab.config(text='JOGGING Z AXIS')
              almStatusLab2.config(text='JOGGING Z AXIS')
            elif jogMode == 3:
              jogMode = 1
              almStatusLab.config(text='JOGGING X & Y AXIS')
              almStatusLab2.config(text='JOGGING X & Y AXIS')
         ...
         ##CARTESIAN ORIENTATION MODE
            ...
            if mainMode != 3:
              mainMode = 3
              jogMode = 1
              almStatusLab.config(text='JOGGING Rx & Ry AXIS')
              almStatusLab2.config(text='JOGGING Rx & Ry AXIS')
            else:                
              jogMode +=1        
            if jogMode == 2:
              almStatusLab.config(text='JOGGING Rz AXIS')
              almStatusLab2.config(text='JOGGING Rz AXIS')
            elif jogMode == 3:
              jogMode = 1
              almStatusLab.config(text='JOGGING Rx & Ry AXIS')
              almStatusLab2.config(text='JOGGING Rx & Ry AXIS')
          ...
          ##J7 MODE
            ...
            mainMode = 4
            almStatusLab.config(text='JOGGING TRACK')
            almStatusLab2.config(text='JOGGING TRACK')
          ##TRACK JOG
      ...
      #else:
        almStatusLab.config(text='XBOX CONTROLLER NOT RESPONDING')
        almStatusLab2.config(text='XBOX CONTROLLER NOT RESPONDING')        
...
def loadProg():
  ...
  #folder = os.path.dirname(os.path.realpath(__file__))
  filetypes = (('robot program', '*.sbot'),("all files", "*.*"))
...
def insertCallProg():  
  ...
  if  str(changeProg[-4:]) != ".sbot":
    changeProg = changeProg + ".sbot"             
...
def calExtAxis():
  ...
  J7negLimLab.config(text=str(-J7NegLim))
  J8negLimLab.config(text=str(-J8NegLim))
  J9negLimLab.config(text=str(-J9NegLim))

  J7posLimLab.config(text=str(J7PosLim))
  J8posLimLab.config(text=str(J8PosLim))
  J9posLimLab.config(text=str(J9PosLim))
...
def zeroAxis7():
  ...
  ser.flushInput()
  time.sleep(.1)
  almStatusLab.config(text="J7 Calibration Forced to Zero")
...
def zeroAxis8():
  ...
  ser.flushInput()
  time.sleep(.1)
  almStatusLab.config(text="J8 Calibration Forced to Zero")
...
def zeroAxis9():
  ...
  ser.flushInput()
  time.sleep(.1)
  almStatusLab.config(text="J9 Calibration Forced to Zero")
...
def roborealm175():
  ...
  while (visfail == 1):
    value = 0
    almStatusLab.config(text="WAITING FOR CAMERA")
    almStatusLab2.config(text="WAITING FOR CAMERA")
    while (value == 0): 
      try:
        f = open(VisFileLoc,"r")
        value = f.readlines()[-1]#.decode()
...
def GCstepFwd():
    GCalmStatusLab.config(text="GCODE READY")
    GCexecuteRow() 
  ...
  if(GcodeFilenameField.get() != ""):
    ...
    if (response[:1] == 'E'):
      ...
      if(response == "P"):
        ...
        GCalmStatusLab.config(text= text)
        status = "no"
        GCread(status)
      elif(response == "F"):
...
def GCread(status):
  ...
  if (response[:1] == 'E'):
    ...
    if(status == "yes"):
      GCalmStatusLab.config(text= "FILES FOUND ON SD CARD:")
    GcodeProgEntryField.delete(0, 'end')
    tab7.gcodeView.delete(0,END)
...
def GCplayProg(Filename):
  GCalmStatusLab.config(text= "GCODE FILE RUNNING")
  def GCthreadPlay():
    global estopActive
    Fn = Filename + ".txt"
    command = "PG"+"Fn"+Fn+"\n"
    ...
    if (response[:1] == 'E'):
      ...
      displayPosition(response)
      if (estopActive == TRUE):
        GCalmStatusLab.config(text= "Estop Button was Pressed")
      else:  
        GCalmStatusLab.config(text= "GCODE FILE COMPLETE") 
    ...
    def GCthreadProg():
      ...
      while tab7.GCrunTrue == 1:
        if (tab7.GCrunTrue == 0):
          ...
          GCalmStatusLab.config(text="GCODE CONVERSION RUNNING")
        GCrowinproc = 1
        GCexecuteRow()
        while GCrowinproc == 1:
          time.sleep(.1)	  
  ...
  if (cmdType == "G"):
    ...
    if (subCmd == "90" or subCmd == "91" or subCmd == "28"):
      ...
      if (response[:1] == 'E'):
        ...
        GCstopProg()
        tab7.GCrunTrue = 0
        GCalmStatusLab.config(text="UNABLE TO WRITE TO SD CARD")   
      else:
        displayPosition(response) 
...
curRowLab.place(x=525, y=120)


almStatusLab = Label(tab1, text = "SYSTEM READY - NO ACTIVE ALARMS")
...
J1negLimLab = Label(J1jogFrame, font=("Arial", 8), text = str(-J1NegLim))
J1negLimLab.place(x=115, y=25)
J1posLimLab = Label(J1jogFrame, font=("Arial", 8), text = str(J1PosLim))
...
J2negLimLab = Label(J2jogFrame, font=("Arial", 8), text = str(-J2NegLim))
J2negLimLab.place(x=115, y=25)
J2posLimLab = Label(J2jogFrame, font=("Arial", 8), text = str(J2PosLim))
...
J3negLimLab = Label(J3jogFrame, font=("Arial", 8), text = str(-J3NegLim))
J3negLimLab.place(x=115, y=25)
J3posLimLab = Label(J3jogFrame, font=("Arial", 8), text = str(J3PosLim))
...
J4negLimLab = Label(J4jogFrame, font=("Arial", 8), text = str(-J4NegLim))
J4negLimLab.place(x=115, y=25)
J4posLimLab = Label(J4jogFrame, font=("Arial", 8), text = str(J4PosLim))
...
J5negLimLab = Label(J5jogFrame, font=("Arial", 8), text = str(-J5NegLim))
J5negLimLab.place(x=115, y=25)
J5posLimLab = Label(J5jogFrame, font=("Arial", 8), text = str(J5PosLim))
...
J6negLimLab = Label(J6jogFrame, font=("Arial", 8), text = str(-J6NegLim))
J6negLimLab.place(x=115, y=25)
J6posLimLab = Label(J6jogFrame, font=("Arial", 8), text = str(J6PosLim))
...
J7negLimLab = Label(J7jogFrame, font=("Arial", 8), text = str(-J7NegLim))
J7negLimLab.place(x=10, y=30)
J7posLimLab = Label(J7jogFrame, font=("Arial", 8), text = str(J7PosLim))
...
J8negLimLab = Label(J8jogFrame, font=("Arial", 8), text = str(-J8NegLim))
J8negLimLab.place(x=10, y=30)
J8posLimLab = Label(J8jogFrame, font=("Arial", 8), text = str(J8PosLim))
...
J9negLimLab = Label(J9jogFrame, font=("Arial", 8), text = str(-J9NegLim))
J9negLimLab.place(x=10, y=30)
J9posLimLab = Label(J9jogFrame, font=("Arial", 8), text = str(J9PosLim))
...
curRowEntryField.place(x=609, y=120)
...
almStatusLab2 = Label(tab2, text = "SYSTEM READY - NO ACTIVE ALARMS")
...
loadAR3But = Button(tab3,  text="Load TEST Defaults",  width=26, command = LoadAR3default)
...
VisBackdropImg = ImageTk.PhotoImage(Image.open(r'GUI_ControlPanelV4\VisBackdrop.png'))
...
GCalmStatusLab = Label(tab7, text = "GCODE IDLE")
...
#stopGPhoto=PhotoImage(file="stop-icon.png")
...
playGPhoto=PhotoImage(file=r"GUI_ControlPanelV4\play-icon.png")
